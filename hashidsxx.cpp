...
static int usage(const char *prog) {
  std::cerr << "USAGE: [encode|decode]" << prog << " [options]" << std::endl
            ...
            << "-m, --min       The minimum amount of characters to use, only useful when encoding" << std::endl
            << "-i, --input     Input numbers, in case of encode repeat this for multiple input numbers" << std::endl;
  return 1;
};
...
int main(int argc, char **argv) {
  ...
  while ((arg = getopt_long(argc, argv, "hs:a:m:i:", g_LongOpts, &optindex)) != -1) {
    ...
    case 'm':
      ...
      break;
    case 'i':
      if (encode) encode_input.push_back(std::stoi(std::string(optarg)));
      else decode_input = std::string(optarg);
      break;
    }
  }
